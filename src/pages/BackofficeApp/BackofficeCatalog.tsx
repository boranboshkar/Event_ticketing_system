...
//TODO: add check if there are no events show a relevant message(sold out)
// if there are no ticket availabe for an event do not show it.
//TODO: show all event even those soldout or finished.

const BackofficeCatalogPage = () => {
  const { navigate } = useContext(BackofficeNavigatorContext);
  const [page, setPage] = React.useState(1);
  const { data, isError, isLoading, error } = useQuery({
    queryKey: ["backoffice_events", page],
    queryFn: () => fetchEvents(page),
  });
  const handleEventClick = (eventId: string) => {
    navigate("evetnDetails", { eventId });
  };
  if (isLoading)
    return (
...
export default BackofficeCatalogPage;
